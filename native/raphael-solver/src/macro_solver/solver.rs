...
impl<'a> MacroSolver<'a> {
    ...
    pub fn solve(&mut self) -> Result<Vec<Action>, SolverException> {
        ...
        self.solve_from_state(initial_state)
    }

    /// CielCraft addition: solve the remainder of a craft starting from an
    /// arbitrary (live) state instead of the synthesis-begin state.
