...
public class NodeSchedulerTests
{
    ...
    /// <summary>A fixed cost table so the arithmetic below does not move with the rotation tables (7.14).</summary>
    private static int StubGpPerNode(NodeKind kind, bool collectable, int maxGp)
    {
        var wanted = collectable ? 600 : kind is NodeKind.Unspoiled or NodeKind.Legendary ? 800 : 500;
    ...
    private static SchedulerOptions Options(bool waitAtHome = false, bool hasHome = false, int waitAtHomeMinutes = 8, bool atHome = false) =>
        new(Lead, TimeSpan.FromHours(3), waitAtHome, waitAtHomeMinutes, hasHome, atHome) { GpPerNode = StubGpPerNode };
