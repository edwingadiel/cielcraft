    ...
    bool AtHome = false)
{
    ...
    /// <summary>
    /// GP a node run wants: (kind, collectable, max GP) → GP. The rotation
    /// tables (7.14) by default; the runner passes the settings-aware
    /// variant so user overrides count, tests pass a fixed table.
    /// </summary>
    public Func<NodeKind, bool, int, int> GpPerNode { get; init; } = GatheringRotationCost.GpPerNode;
